...
function ModalNewTechnology({ modalOpen, abrirFecharModal }) {
 ...
 function registerTech(data){
  ...
  .catch(() => {
     ...
     toast.error("Uma tecnologia com esse nome já foi cadastrada, tente outro!"); 
  });
  ...
  return (
    ...
    >
      ...
      <form className="formModal"  onSubmit={
          handleSubmit(registerTech)}>
        <div className="inputsTechnology">
          <label className="labelTechnology" htmlFor="nameNewTecnology">
            Nome
          </label>
          <input className="inputNewTechnology" id="nameNewTecnology"   {...register('title')} />
          ...
          <select className="inputNewTechnology" id="statusNewTecnology"    {...register('status')}>
        ...
        <button className="cadastroTechnologyBnt" type="submit">Cadastrar Tecnologia</button>
      </form>
    </Modal>
  );
}
